...
def load_db(filename):
    if os.path.exists(filename):
        ...
        st.sidebar.warning(f"⚠️ Файл бази {filename} не знайдено.")
        return {}
...
if st.button("Запустити перевірку з AI", type="primary", use_container_width=True):
    if not rmpd_file or not cmr_files:
        ...
        st.error("❌ Перевищено ліміт файлів! Завантажте не більше 5 CMR та 5 Інвойсів.")
    else:
        with st.spinner("Перевірка виконується... Це може зайняти 15-20 секунд."):
            ...
            if rmpd_data and ocr_data:
                ...
                # Візуалізація
                st.success("✅ Перевірку успішно завершено!")
                ...
                with tab1:
                    ...
                    for item in report:
                        ...
                        is_success = "✅" in status_str or "Збіг" in status_str or "Логічно" in status_str
                        is_warning = "⚠️" in status_str or "Попередження" in status_str
