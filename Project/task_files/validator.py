...
API_KEY = "AIzaSyDtzTvik-vmnfMg1K-xOklWU7ZsH4tZblI"
...
def validate_documents(rmpd_data, ocr_data, ecmt_db=None, gps_db=None):
    ...
    for field in fields_to_compare:
        ...
        if similarity == 100.0:
            ...
            status = "⚠️ Попередження (можлива опечатка OCR)"
        else:
            status = "❌ Помилка"
    ...
    if ("Transit" in str(road_type) or "Транзит" in str(road_type)) and entrance and exit_point:
        ...
        report.append({
            ...
            "status": "✅ Логічно" if is_logical else "❌ Підозріло"
        })
    ...
    if start_inside:
        ...
        if sim >= 80.0:
            ...
            status = "⚠️ Попередження (перевірте адресу)"
        else:
            status = "❌ Помилка"
    ...
    if permission_type == "EKMT permit":
        ...
        if ecmt_db and ecmt_number in ecmt_db:
            ...
            report.append({
                ...
                "similarity": sim_from,
                "status": "✅ Збіг" if sim_from == 100.0 else "❌ Помилка"
            })
            ...
            report.append({
                ...
                "similarity": sim_to,
                "status": "✅ Збіг" if sim_to == 100.0 else "❌ Помилка"
            })
        else:
            report.append({
        ...
        if legal_basis:
            ...
            report.append({
                ...
                "status": "✅ Логічно" if is_valid else "❌ Підозріло"
            })
        else:
            report.append({
    ...
    if gps_db is not None:
        ...
        if expected_locator and expected_locator == locator_number:
            ...
            report.append({
                ...
                "similarity": 0.0,
                "status": "❌ Помилка (Невідповідність)"
            })
